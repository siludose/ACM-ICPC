...
bool cmp(const point &a,const point &b ){if(a.y==b.y)return a.x<b.x;return a.y<b.y;}
int Graham()//按逆时针排序的
{
	int i;
	std::sort(p,p+n,cmp);
...
double rc_tri()//返回值处以2就是面积
{
	...
	for(i=0;i<n;i++)
	{
		while(xmult(save[c+1],save[b],save[i])-(temp=xmult(save[c],save[b],save[i]))>eps)
			c=(c+1)%n;
		maxx=max(maxx,temp);
		while(xmult(save[c],save[b+1],save[i])-(temp=xmult(save[c],save[b],save[i]))>eps)
			b=(b+1)%n;
		maxx=max(maxx,temp);
	}
	return maxx;
